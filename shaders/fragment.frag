    ...
    for(;;){
		...
		if(int(block[0]*255)!=1){
			...
			case 28:{
				...
				switch(pointer/tex3DszLimit){
				case 0:
					FragColor.rgb = texelFetch(lmap,crd,0).rgb * block.gba * 32767.0 / (brightness+1.0);
					break;
				case 1:
					FragColor.rgb = texelFetch(lmap2,crd,0).rgb * block.gba * 32767.0 / (brightness+1.0);
					break;
				case 2:
					FragColor.rgb = texelFetch(lmap3,crd,0).rgb * block.gba * 32767.0 / (brightness+1.0);
					break;
				case 3:
					FragColor.rgb = texelFetch(lmap4,crd,0).rgb * block.gba * 32767.0 / (brightness+1.0);
					break;
				}
				ePos = getRayPos();
				calcFog();
